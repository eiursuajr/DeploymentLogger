...
namespace MyGeneration.dOOdads
{
	...
	{		
        ...
        static public string StaticConnectionString
        {
            set { _staticConnectionString = value; }
            get { return _staticConnectionString; }
        }

        static private Stack ConnectionStringStack = new Stack(); 

        static public void PushStaticConnectionString()
        {
            ConnectionStringStack.Push(StaticConnectionString);
            StaticConnectionString = string.Empty;
        }

        static public void PopStaticConnectionString()
        {
            if (ConnectionStringStack.Count > 0)
            {
                StaticConnectionString = Convert.ToString(ConnectionStringStack.Pop());
            }
        }

        static public void ClearConnectionString()
        {
            ConnectionStringStack.Clear();
            StaticConnectionString = string.Empty;
        }
        ...
        private static string _staticConnectionString = string.Empty;
		internal string _config = BusinessEntity._defaultConfig;
		internal IDbConnection _notRecommendedConnection = null;
		internal string _raw = BusinessEntity._staticConnectionString;
